...
function  [DynOpt, params] = Observer_EKF_att_v1(DynOpt, params)
    ...
    for k = 1:DynOpt.ObserverTest.Nagents
        ...
        xhat_now_full = tmp_full.y(:,end);
        tmp = hmap_attitude_v1(pos_full,xhat_now_full, DynOpt);
        
        if DynOpt.ObserverTest.nMagneto == 0
            z_hat = tmp.y_Gyro(1+3*(k-1):3+3*(k-1),end);
        elseif DynOpt.ObserverTest.nMagneto == 1
            z_hat = [ tmp.y_Mag(1+6*(k-1):3+6*(k-1),end); tmp.y_Gyro(1+3*(k-1):3+3*(k-1),end)];
        elseif DynOpt.ObserverTest.nMagneto == 2
            z_hat = [ tmp.y_Mag(1+6*(k-1):6+6*(k-1),end); tmp.y_Gyro(1+3*(k-1):3+3*(k-1),end)];
        end
        

        %%%% Linearisation %%%%
        % Linearized State equation in xk-1
        G = Gmatrix_EKF_att_v1(DynOpt,params,x_past,k); 

        % Linearized State equation in xk 
        H = Hmatrix_EKF_att_v1(DynOpt,params,xhat_now,z_hat,k);
