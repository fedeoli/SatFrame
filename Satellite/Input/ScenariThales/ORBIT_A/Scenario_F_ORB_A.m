...
tfinal = 10*T;                                                         % simulation's final time [s]
...
% Scenario F Specifications (Cross-track semi-major axis 250m)
x0 = 0.25;
y0 = 0.5;
z0 = 1;

% Initial conditions given in terms of relative coordinates: (x0, y0, z0, x0_dot , y0_dot , z0_dot)
deputy_rel0_LVLH(1, 1:6) = [x0; 0; 0; 0; -y0*n; z0*n];                                                                                                              % first deputy
deputy_rel0_LVLH(2, 1:6) = [x0*cos(120*pi/180); -y0*sin(120*pi/180); z0*sin(120*pi/180); -x0*n*sin(120*pi/180); -y0*n*cos(120*pi/180); z0*n*cos(120*pi/180)];    % second deputy
deputy_rel0_LVLH(3, 1:6) = [x0*cos(240*pi/180); -y0*sin(240*pi/180); z0*sin(240*pi/180); -x0*n*sin(240*pi/180); -y0*n*cos(240*pi/180); z0*n*cos(240*pi/180)];    % third deputy
...
% T_horizon = tfinal;
params.DT_burn = T_horizon/(T_horizon/T*15);                                  % The impulsive DT will be applied every DT_burn. Suggestion: select odd fractions of the period
% params.DT_burn = MaximumBurningTime;
...
% Scenario F Specifications (Finish: Leader Follower at 5km)
x0 = 0;
y0 = 5;
z0 = 0;

% Second satellite (first deputy)
params.sat(2).TrajectoryProfile = @LeaderFollower_V1_1;
params.sat(2).xRef = x0;
params.sat(2).yRef = y0 - 0.2;
...
params.sat(3).TrajectoryProfile = @LeaderFollower_V1_1;
...
params.sat(4).TrajectoryProfile = @LeaderFollower_V1_1;
params.sat(4).xRef = x0;
params.sat(4).yRef = y0 + 0.2;
