...
params.CollisionAvoidance = 0;              % params.CollisionAvoidance = 1 : The Collision Avoidance control calculation is activated
...
params.CollisionAvoidanceStrategy = 'SafetyParking';         % params.CollisionAvoidanceStrategy = 'Potential' : An exponential control is applied based on the repulsive potential between two 
                                                             ...
                                                             % params.CollisionAvoidanceStrategy = 'SafetyParking' : The colliding deputy exerts an optimal Delta V which sends him in a
                                                             %                                                       safe parking orbit for a fixed amount of time after which it can resume its 
                                                             %                                                       nominal mode.
...
params.CA_GlobalSafeMode = 1;               % params.CA_GlobalSafeMode = 1 : If a Collision is spotted ahead in time, each deputy stops maneuvering and enters CA_mode
                                            ...
                                            % IMPORTANT NOTE: The 'GlobalSafeMode' only works if the 'SafetyParking' CA strategy is selected. 
...
% Activation of Pulse Width Modulation (PWM) - impulsive and fixed control (only for Continuous Control)
params.PWM = 0;                             % params.PWM = 1 : the control evaluated by the (A)LQR is translated into impulses of the same intensity
...
%         deputy_rel0_LVLH(3, 1:6) = [0; -1.5; 0; 0; 0; 0];                  % second deputy
        ...
        % Optimization testing (F.Oliva)
        deputy_rel0_LVLH(1, 1:6) = [-0.5; -1.5; 0; 0; 0; 0];                  % first deputy
        deputy_rel0_LVLH(2, 1:6) = [-1.5; 1; 0; 0; 0; 0];                  % second deputy
        deputy_rel0_LVLH(3, 1:6) = [1.3; 0.9; 0; 0; 0; 0];                  % third deputy
        ...
        for i = 1:N_deputy
            ...
            satellites_iner_ECI(1 + 6*i : 6*(i + 1), 1)  = rel2iner_V2_1(deputy_rel0_LVLH(i, 1:6), chief_iner_ECI, chief_OOE, params);
...
params.sat(1).MissionAttitude = 'Nadir pointing x';
...
params.sat(2).MissionAttitude = 'Nadir Pointing y';                 % Nadir pointing x; Nadir pointing y; Nadir pointing z; No maneuver; Hold Attitude
