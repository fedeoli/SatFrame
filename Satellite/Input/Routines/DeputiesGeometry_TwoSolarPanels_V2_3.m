...
for i = 1:N_deputy
    ...
    % 9th surface properties - Solar Panel 2 (1st surface)
    params.sat(i+1).aero_prop(9).n = [1 0 0];
    params.sat(i+1).aero_prop(9).A = l_d*h_d;
    params.sat(i+1).aero_prop(9).arm = [0; -(w_d/2 + h_d/2); 0];
    
    % 10th surface properties - Solar Panel 2 (2nd surface)
    params.sat(i+1).aero_prop(10).n = [-1 0 0];
    params.sat(i+1).aero_prop(10).A = l_d*h_d;
    params.sat(i+1).aero_prop(10).arm = [0; -(w_d/2 + h_d/2); 0];
