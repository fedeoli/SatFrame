...
if strcmpi(params.ControlStrategy, 'Continuous') % Continuous Control Parameters
    ...
    params.FiringThreshold = 1e-9;%1e-8;  % [km/s^2]
    
    % Time to wait until each subsequent burn application (Impulsive case)
    WaitingTime = pi/n;
    
    % Waiting Time cannot be smaller than Parking Time
    if WaitingTime < ParkingTime
        
        WaitingTime = ParkingTime;
        
    end
    
    params.ParkingTime = ParkingTime;
