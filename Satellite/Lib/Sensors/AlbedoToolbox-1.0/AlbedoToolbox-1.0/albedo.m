...
function [a_tot,phi_sat] = albedo(sat,sun,refl,type)
...
%sunsph = [0 1.17 CONST.ua]; % from bhander thesis
%sunsph(3) = sunsph(3)+CONST.ua;
...
% Distance to sat from grid
[grid_theta, grid_phi] = idx2rad(row,col,sy,sx);
[grid(1), grid(2), grid(3)] = sph2cart(grid_theta,pi/2-grid_phi,CONST.EMR+CONST.ua);
satdist = norm(sat-grid);
% Angle to sat from grid
phi_sat = acos(((sat-grid)/satdist).*grid/norm(grid))';
