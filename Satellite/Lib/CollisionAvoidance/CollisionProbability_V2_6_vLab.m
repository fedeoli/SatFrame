function [prob, prob_CA_chief, params] = CollisionProbability_V2_6_vLab(x_est, P, params, u)

%   CollisionProbability_V2_6.m
...
%                     xx = reshape(x, [1,1,length(x)]);

%                     prob(i,j) = ntrapz(z, p2, 1);
                    prob(i,j) = trapz(z, trapz(y, trapz(x, F, 1), 2), 3);
                    
%                     if prob(i,j)*100 > params.CollisionProbabilityThreshold
%                         
%                         keyboard;
%                        q = (max(max(max(F))));
%                        q2 = (1/((2*pi)^(3/2)*sqrt(det(P_rel))))*exp(-0.5*(inv_Prel(1,1)*x_rel(1).^2 + inv_Prel(2,2)*x_rel(2).^2 + inv_Prel(3,3)*x_rel(3).^2 + 2*inv_Prel(2,1)*x_rel(1).*x_rel(2) + 2*inv_Prel(3,1)*x_rel(1).*x_rel(3) + 2*inv_Prel(3,2)*x_rel(2).*x_rel(3)));
%                        pp = q2*6*sqrt(P1(1,1))*6*sqrt(P1(2,2))*6*sqrt(P1(3,3));
%                         
%                     end
%                     p1 = trapzf2(x, F);
%                     p2 = ntrapz(y, p1, 2);
...
%                     tic
%                     fun = @(x,y,z) (1/((2*pi)^(3/2)*sqrt(det(P_rel))))*exp(-0.5*(inv_Prel(1,1)*x.^2 + inv_Prel(2,2)*y.^2 + inv_Prel(3,3)*z.^2 + 2*inv_Prel(2,1)*x.*y + 2*inv_Prel(3,1)*x.*z + 2*inv_Prel(3,2)*y.*z));
%                     q = triplequad(fun, x_rel(1) - 3*sqrt(P1(1,1)), x_rel(1) + 3*sqrt(P1(1,1)), x_rel(2) - 3*sqrt(P1(2,2)), x_rel(2) + 3*sqrt(P1(2,2)), x_rel(3) - 3*sqrt(P1(3,3)), x_rel(3) + 3*sqrt(P1(3,3)));
%                     toc
        ...
        for i = 1:N_deputy
            ...
            % Collision probability over the integrated volume
            prob_CA_chief(i) = trapz(z, trapz(y, trapz(x,F,1), 2), 3);
            
%             if prob_CA_chief(i)*100 > params.CollisionProbabilityThreshold
%                         
%                 keyboard;
%                        q = (max(max(max(F))));
%                        q2 = (1/((2*pi)^(3/2)*sqrt(det(P_rel))))*exp(-0.5*(inv_Prel(1,1)*x_rel(1).^2 + inv_Prel(2,2)*x_rel(2).^2 + inv_Prel(3,3)*x_rel(3).^2 + 2*inv_Prel(2,1)*x_rel(1).*x_rel(2) + 2*inv_Prel(3,1)*x_rel(1).*x_rel(3) + 2*inv_Prel(3,2)*x_rel(2).*x_rel(3)));
