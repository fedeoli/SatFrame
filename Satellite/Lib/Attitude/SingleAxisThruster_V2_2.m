function [u_out, satellites_iner_ECI, params] = SingleAxisThruster_V2_2(t, u, satellites_iner_ECI, deputy_rel_LVLH, satellites_attitude, params)

%   SingleAxisThruster_V2_2.m
...
if  strcmpi(params.ControlStrategy, 'Continuous')
    ...
    for j = 1:N_deputy
        ...
        if 2*abs(acos(abs(q_err(1))))*180/pi < params.deputy(j).att_threshold
...
elseif strcmpi(params.ControlStrategy,  'Impulsive')
    ...
    for j = 1:N_deputy
        ...
        quat_ref = eul2quat(params.DesiredAttitude(:,j)', 'ZYX');
        q_err = QuaternionError_V2_1(quat_Hill, quat_ref);
        params.deputy(j).check_attitude = abs(acos(abs(q_err(1))))*180/pi < params.deputy(j).att_threshold;

        if (params.deputy(j).check_attitude == 1) && (params.deputy(j).Burn_executed == 0)
            ...
            params.deputy(j).WaitingTime = t + params.DT_burn;
            params.deputy(j).Burn_executed = 1;
