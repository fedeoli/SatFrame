function [u_out, satellites_iner_ECI, params] = SingleAxisThruster_V2_5(t, u, satellites_iner_ECI, deputy_rel_LVLH, params)

%   SingleAxisThruster_V2_5.m
...
%
%   20200518 V2_4:
%   - Solved an issue for which the Delta V would have been zero in the ideal case (params.Attitude = 0 and params.RealThruster = 0)
%
%   20200519 V2_5:
...
% Check if we are in the ideal case
if ~(params.Attitude == 0 && params.RealThruster == 0)
    ...
    if  strcmpi(params.ControlStrategy, 'Continuous') && t >= params.ParkingTime
        ...
        for j = 1:N_deputy
            ...
            if params.u_module(j+1) > 0 && params.Attitude
                ...
                quat_ECI = params.SatellitesAttitude(1 + 7*j: 4 + 7*j)';
                quat_ECI_conv = ConvertQuat_V2_1(quat_ECI, 'ScalarTo4');
                R_ECI2Body = RotationConversion_V2_1('QtoDCM', quat_ECI_conv);
                R_ECI2Hill = RECI2Hill(vect_r, vect_v);
                R_Hill2Body = R_ECI2Body*R_ECI2Hill';
                quat_Hill = RotationConversion_V2_1('DCMtoQ', R_Hill2Body);
                quat_Hill = ConvertQuat_V2_1(quat_Hill, 'ScalarTo1');
                
                % Extract desired attitude and convert to quaternions
                quat_ref_conv = RotationConversion_V2_1('EA321toQ', params.DesiredAttitude(:,j+1)'*180/pi);
                quat_ref = ConvertQuat_V2_1(quat_ref_conv, 'ScalarTo1');
    ...
    elseif strcmpi(params.ControlStrategy,  'Impulsive') && ( t >= params.ParkingTime || ( params.CollisionFlag == 1 || params.CollisionHolding || sum([params.sat.CA_Maneuvering]) ~= 0 ) )
        ...
        for j = 1:N_deputy
                ...
                if params.sat(j+1).check_attitude == 1 && params.sat(j+1).Burn_executed == 0
                    ...
                    if params.RealThruster
                        ...
                        if params.sat(j+1).FiniteBurnTime > 0
                            ...
                            params.DV(:, round(t/params.time_step), j) = u_out(:,j)*params.time_step;
            ...
            elseif params.Attitude == 0 && params.RealThruster && params.sat(j+1).Burn_executed == 0
                ...
                if params.sat(j+1).FiniteBurnTime > 0
                    ...
                    if DV > 0 % if it is a new impulse, compute the direction. Otherwise, use the stored one (i.e., impulse previously calculated and not yet fully executed)
                ...
                zeri = [0; 0; 0];
                params.DV(:, round(t/params.time_step), j) = zeri;
