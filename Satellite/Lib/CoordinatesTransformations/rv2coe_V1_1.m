function coe = rv2coe_V1_1(vect_r, vect_v, mi)
...
if h_xy < 1e-10 || e < 1e-10  % equatorial or circular orbit
   
   if e > 1e-10 % equatorial and non cicrcular orbit 
       
	  cos_argp = vect_e(1)/e;
	  sin_argp = vect_e(2)/e;
	  coe(4) = atan2(sin_argp, cos_argp);
  
   else % circular orbit (undefined argument of the perigee)
       
	  coe(4)=0;
      
   end
   
else % neither equatorial nor circular orbit
...
if h_xy > 1e-10 % non-equatorial orbit
