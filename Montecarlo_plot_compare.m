...
function H = Montecarlo_plot_compare(pathname,initperc)
    ...
    a = dir(strcat(path_local,'/UKF/*.mat'));
    nelem = numel(a)-1;
    
    % plot window
    load(strcat(path_local,'/UKF/simulation_',num2str(1),'.mat'))
    ...
    if 0
        % select random shot
        shot = randi(nelem);
        load(strcat(path_local,'/UKF/simulation_',num2str(shot),'.mat'))
        ...
        load(strcat(path_local,'/GPS_UKF/simulation_',num2str(shot),'.mat'))
        semilogy(time_interval,DynOpt.out(1).errnorm_pos(window_interval),'--','LineWidth',2);
    end
    
    %%%%%%%%%%% DISPERSION ANALYSIS %%%%%%%%%%%
    load(strcat(path_local,'/UKF/recap.mat'))
    Montecarlo_UKF = Montecarlo_data;
    load(strcat(path_local,'/GPS_UKF/recap.mat'))
